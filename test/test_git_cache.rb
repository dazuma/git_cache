...
describe ::GitCache do
  ...
  describe "with local git" do
    let(:exec_tool) { ::ExecService.new }
    let(:git_repo_dir) { ::File.join(::Dir.tmpdir, "git_cache_test3") }
    ...
    it "makes source files writable if the environment variable is set" do
      ...
      commit_file(file_name)
      begin
        ::ENV["GIT_CACHE_WRITABLE"] = "true"
        found_path = git_cache.get(local_remote, path: file_name)
        ::File.write(found_path, "changed")
      ensure
        ::ENV["GIT_CACHE_WRITABLE"] = nil
      end
    end
  ...
  describe "with two local git repos" do
    let(:exec_tool) { ::ExecService.new }
    let(:git_repo_dir1) { File.join(Dir.tmpdir, "git_cache_test3") }
    let(:git_repo_dir2) { File.join(Dir.tmpdir, "git_cache_test4") }
  ...
  describe "RepoInfo" do
    ...
    def make_repo_info(data)
      ::GitCache::RepoInfo.new(base_dir, data)
    end
  ...
  describe "SourceInfo" do
    ...
    def make_source_info(git_path, data = {})
      ::GitCache::SourceInfo.new(base_dir, sha, git_path, data)
    end
    ...
    it "compares first by sha then by git_path" do
      sha1 = "a" * 40
      sha2 = "b" * 40
      info_a1 = ::GitCache::SourceInfo.new(base_dir, sha1, "aaa", {})
      info_a2 = ::GitCache::SourceInfo.new(base_dir, sha1, "bbb", {})
      info_b1 = ::GitCache::SourceInfo.new(base_dir, sha2, "aaa", {})
  ...
  describe "with github_integration" do
    ...
    before do
      skip "Skipped integration test" unless ENV["TEST_INTEGRATION"]
    end
